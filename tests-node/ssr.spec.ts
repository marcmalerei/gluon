...
describe('@gluonjs/ssr static output and style transport', () => {
  ...
  it('emits safe ordered style carriers, assets, hints, and mixed static/dynamic output', async () => {
    ...
    expect(response.styles.entries).toHaveLength(9);
    expect(response.styles.entries.map((entry) => entry.id)).toEqual([
      'gluon-ui-layer-order',
      'gluon-ui-foundation',
      'gluon-ui-tokens',
    ...
    expect(response.styles.entries.map((entry) => entry.order)).toEqual([0, 1, 2, 3, 4, 5, 6, 7, 8]);
    expect(response.styles.entries[4]?.cssText).toContain('.gluon-button');
    expect(response.styles.entries[5]?.cssText).toContain('.gluon-status-badge');
    expect(response.styles.entries[6]?.cssText).toContain('.shop-editorial-link');
    expect(response.styles.entries[7]?.cssText).toContain('--gluon-color-action: #c8ff00');
    expect(response.styles.entries[8]?.cssText).toContain('.checkout-page');
