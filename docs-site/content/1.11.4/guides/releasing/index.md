...
The `1.11.4` documentation describes the prepared lockstep release candidate.
All 21 official manifests are at `1.11.4`; it is not a published package train
...
[#470](https://github.com/marcmalerei/gluon/issues/470) carry the patch release
that corrects the VSIX language-server version handshake after the immutable
`v1.11.0` candidate stopped before publication.
Issue [#473](https://github.com/marcmalerei/gluon/issues/473) refreshes the
VSIX clean-install integrity after the immutable, unpublished `v1.11.1`
candidate stopped before publication on a changed registry tarball checksum.
Issue [#477](https://github.com/marcmalerei/gluon/issues/477) re-resolves the
complete VSIX Azure lock graph after the immutable, unpublished `v1.11.2`
candidate stopped before publication because its locked
`@azure/core-auth@1.11.1` tarball was no longer available. The exact VSIX
clean install, package build, and bundled language-server smoke validation now
run in Quality Gates with an isolated npm cache before a tag can be created.
Issue [#479](https://github.com/marcmalerei/gluon/issues/479) makes the
post-publish npm visibility and `latest`-tag probes online-preferred and
bounded-retry tolerant, so normal registry propagation cannot leave an
otherwise published train stuck before immutable-release finalization.
The release cut retains a Quality-Gates-tested candidate followed only by its
release-cut evidence and compatibility manifest.
...
No `v1.11.4` tag or immutable GitHub release exists while this candidate is
...
reproducible package-content digests, clean-installs all 21 local archives,
...
21 reviewed versions directly under `latest` with provenance and without a
