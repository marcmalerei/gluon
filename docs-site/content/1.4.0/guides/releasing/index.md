...
The `1.4.0` documentation describes the prepared lockstep candidate. All 18
official manifests are at `1.4.0`. Registry preflight on 2026-07-24 confirmed
that every contracted package exposes `1.3.0` as `latest` and that `1.4.0` is
absent. Immutable GitHub release `v1.3.0` remains the current finalized release
during candidate review. The `v1.0.9` GitHub release remains a draft after its
...
reproducible package-content digests, clean-installs all 18 local archives,
...
18 reviewed versions directly under `latest` with provenance and without a
