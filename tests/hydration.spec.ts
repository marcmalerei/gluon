...
import { Button, Checkbox, Radio, Select, Textarea, buttonStyles, checkboxStyles, radioStyles, selectStyles, textareaStyles } from '@gluonjs/atoms';
...
describe('SSR hydration', () => {
  ...
  it('hands component carriers to their exact renderer-owned sheets and diagnoses mismatches', async () => {
    ...
    })}${Textarea({ value: 'Hydrated note', attributes: { 'aria-label': 'Hydrated note' } })}${Checkbox({ checked: true, attributes: { 'aria-label': 'Hydrated choice' } })}${Radio({ checked: true, name: 'finish', attributes: { 'aria-label': 'Hydrated radio' } })}${Card({ title: 'Hydrated card' })}</main>`;
    const prepared = await prepareForHydration(value);
