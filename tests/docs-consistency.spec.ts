...
describe('docs consistency helpers', () => {
  test('finds only release-line versions instead of unrelated historical examples', () => {
    expect(findLockstepVersionMentions(
      'Part of the lockstep Gluon `1.4.0` release line; use the current `1.12.2` release line. Migration started at `1.0.0`.',
    )).toEqual(['1.4.0', '1.12.2']);
  });
  ...
  test('rejects stale release lines and contradictory current-state wording', () => {
    ...
    ], { currentVersion: '1.12.2', releaseLinePaths: ['packages/router/README.md'] })).toThrow(/stale lockstep version mentions/i);

    expect(() => validateDocsConsistency([
      { path: 'README.md', text: 'The runtime exists, but the API remains experimental.' },
    ], { currentVersion: '1.12.2' })).toThrow(/contradictory release-state wording/i);
  });
  ...
  test('requires policy language on its owning page and accepts a consistent set', () => {
    const entries = [
      ...
      { path: 'packages/store/README.md', text: 'The package ships as part of the current `1.12.2` release line.\n## Stability notes' },
    ];
