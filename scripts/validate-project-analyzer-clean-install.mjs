...
try {
  const archives = [];
  for (const packageDirectory of ['packages/compiler', 'packages/language-server']) {
  ...
  `);
  await execFile('npm', ['install', ...archives, '--ignore-scripts', '--no-audit', '--no-fund'], {
  ...
  console.log('project analyzer clean install valid: packed local compiler, CLI, schema, public API, and zero-config report');
} finally {
