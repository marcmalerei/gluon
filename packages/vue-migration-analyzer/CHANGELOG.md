...
- Advanced the lockstep release with the generated package documentation and
  updated compatibility support matrix.
...
- Reported lockstep framework version `1.2.0` in analyzer output while
...
- Reported lockstep framework version `1.0.10` in analyzer output while
  retaining the existing report-only analysis contract.
...
- Reported lockstep framework version `1.0.8` in analyzer output while
  retaining the existing report-only analysis contract.
...
- Included the unchanged analyzer runtime in the lockstep `1.0.1` recovery
