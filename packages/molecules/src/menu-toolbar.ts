...
export const ContextMenu = defineMolecule(({ children, ...props }: ContextMenuProps) => q.div({ class: 'gluon-context-menu-target', onContextMenu: (event: MouseEvent) => { event.preventDefault(); (event.currentTarget as HTMLElement).querySelector<HTMLElement>('[role="menu"]')?.removeAttribute('hidden'); props.onOpenChange?.(true, event); }, children: [children, renderMenu({ ...props, open: false })] }), 'ContextMenu', [menuToolbarStyleDependency]);
