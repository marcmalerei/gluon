...
export const menuToolbarStyles = css`
  @layer molecules {
    :where(.gluon-menu, .gluon-context-menu, .gluon-menubar) {
    ...
    :where(.gluon-menu-trigger, .gluon-menu-item) {
      min-block-size: 44px;
    ...
    :where(.gluon-menu-item:hover, .gluon-menu-item:focus-visible) { background: var(--gluon-menu-active); outline: var(--gluon-focus-width, 3px) solid var(--gluon-color-focus, #173f91); outline-offset: -3px; }
    ...
    :where(.gluon-menu-separator) { display: block; flex: 0 0 1px; block-size: 1px; margin: .25rem; background: var(--gluon-menu-border); }
    ...
    @media (max-width: 390px) {
      ...
      :where(.gluon-menubar) { flex-wrap: wrap; }
    }
    @media (forced-colors: active) {
      :where(.gluon-menu-surface) { border-color: CanvasText; box-shadow: none; }
      :where(.gluon-menu-item:focus-visible) { background: Highlight; color: HighlightText; outline-color: Highlight; }
      :where(.gluon-menu-item[aria-checked="true"])::before { color: Highlight; }
    }
    @media (prefers-reduced-motion: reduce) {
      :where(.gluon-menu *, .gluon-context-menu *, .gluon-menubar *) { scroll-behavior: auto; transition-duration: 0s !important; animation-duration: 0s !important; }
    }
  }
`;
