...
interface ToolbarItemBase { readonly id: string; readonly label: TemplateValue; readonly disabled?: boolean; }
...
function callListener<EventType extends Event>(listener: ((event: EventType) => unknown) | { handleEvent(event: EventType): unknown } | null | undefined, event: EventType): void {
...
function renderToolbar(props: ToolbarProps): TemplateResult {
  ...
  return q.div({
    ...
    children: props.items.map((item) => {
      ...
      const common = { ...attributes, id: toolbarItemId(props.id, item.id), class: ['gluon-toolbar-item', item.attributes?.class], part: 'item', tabIndex: item === firstEnabled ? 0 : -1, data: { ...item.attributes?.data, gluonToolbarItem: true, value: item.id }, aria: { ...item.attributes?.aria, disabled: item.disabled || undefined }, onClick: (event: MouseEvent) => { callListener(onClick, event); if (event.defaultPrevented || item.disabled) { event.preventDefault(); return; } item.onActivate?.(event); }, children: item.label };
