...
export const quarkManifest = Object.freeze({
  ...
  entries: Object.freeze([
    ...
    { name: 'Popover', kind: 'component', status: 'stable', accessibility: 'Uses the native popover attribute; the trigger remains responsible for popovertarget and accessible naming. focusable=false omits the default programmatic focus target.', extension: 'attributes targets HTMLDivElement; id, children, popover mode, and focusability stay explicit.', ...sharedEvidence },
    { name: 'Tooltip', kind: 'component', status: 'stable', accessibility: 'Applies aria-describedby to the caller-rendered trigger and emits non-focusable role=tooltip content. Mouse, keyboard focus, touch, Escape, outside dismissal, delay, and nested ownership are supported.', extension: 'The trigger renderer must apply owned attributes to its native control. hostAttributes and contentAttributes are separate; positioning and behavior keys are rejected.', example: 'docs-site/examples/ui-system.ts', tests: ['tests/ui-system.spec.ts', 'tests/anchored-overlays.spec.ts', 'tests-node/ssr.spec.ts', 'tests/hydration.spec.ts'] },
    { name: 'HoverCard', kind: 'component', status: 'stable', accessibility: 'Applies aria-controls, aria-expanded, and aria-haspopup=dialog to the caller-rendered trigger. Focus remains on the trigger until ArrowDown or Enter intentionally enters labelled dialog content.', extension: 'The trigger renderer must apply owned attributes to its native control. hostAttributes and contentAttributes are separate; positioning and behavior keys are rejected.', example: 'docs-site/examples/ui-system.ts', tests: ['tests/ui-system.spec.ts', 'tests/anchored-overlays.spec.ts', 'tests-node/ssr.spec.ts', 'tests/hydration.spec.ts'] },
