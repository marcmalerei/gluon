...
test('customer flow, persistence, SSR hydration, navigation, accessibility, and HMR', async ({ page }) => {
  await page.goto('/');
  await expect(page.locator('[data-client-ready]')).toHaveAttribute('data-client-ready', 'true');
  ...
  const path = new URL('../app/hmr-marker.ts', import.meta.url);
  const original = await readFile(path, 'utf8');
  try {
    await writeFile(path, original.replace('baseline', 'updated'));
    await expect(page.locator('[data-hmr-marker]')).toHaveText('updated');
    await expect(page.getByLabel('Email')).toHaveValue('buyer@example.test');
